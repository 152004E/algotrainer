...
**Commits:** Only the user makes commits. Subagents and the controller never run `git commit`/`git add`/`git push` without explicit user request. When a task's work is ready, report "listo para commitear" and let the user commit.
